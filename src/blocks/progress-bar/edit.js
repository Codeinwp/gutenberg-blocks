...
const ProgressBar = ({ attributes, setAttributes, className, isSelected, clientId, name, toggleSelection }) => {
	useEffect(() => {
		initBlock();
	}, []);
	...
	const onHeightChange = (value) => {
		if (30 > value) {
			...
			setAttributes({
				...
				titleStyle: 'outer',
				percentagePosition:
					'append' === attributes.percentagePosition || 'default' === attributes.percentagePosition
						? 'outer'
						: attributes.percentagePosition,
			});
		} else {
			if (heightMode.isAutomatic) {
				setHeightMode({
			...
			setAttributes({
				...
				percentagePosition: heightMode.isAutomatic
					? heightMode.percentagePosition
					: attributes.percentagePosition,
				height: value,
			});
		}
				...
				>
					...
					>
						{('default' === attributes.titleStyle || 'highlight' === attributes.titleStyle) && (
							<div
								className={classnames('wp-block-themeisle-blocks-progress-bar__area__title', {
