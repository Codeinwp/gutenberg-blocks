...
const { Disabled, Placeholder, Spinner } = wp.components;
...
const Edit = ({ attributes, setAttributes, className }) => {
	const { posts, categoriesList, authors } = useSelect(
		(select) => {
			const catIds =
				attributes.categories && 0 < attributes.categories.length
					? attributes.categories.map((cat) => cat.id)
					: [];
	...
	if (0 === posts.length) {
		return (
			<Fragment>
				<Placeholder>{__('No Posts')}</Placeholder>
