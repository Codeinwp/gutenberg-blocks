...
const Edit = ({ attributes, setAttributes, className, isSelected, name, clientId }) => {
	const { hasParent, parentAttributes, isLastChild } = useSelect((select) => {
		...
		return {
			...
			isLastChild: parentBlock
				? clientId === parentBlock.innerBlocks[parentBlock.innerBlocks.length - 1].clientId
				: true,
		};
	}, []);
	...
	const hoverStyles = css`
		&:hover {
			...
			`box-shadow: ${attributes.hoverBoxShadowHorizontal}px ${attributes.hoverBoxShadowVertical}px ${
				attributes.hoverBoxShadowBlur
			}px ${attributes.hoverBoxShadowSpread}px ${hexToRgba(
				attributes.hoverBoxShadowColor ? attributes.hoverBoxShadowColor : '#000000',
	...
	return (
		<Fragment>
			<Controls attributes={attributes} setAttributes={setAttributes} isSelected={isSelected} />

			<Inspector attributes={attributes} setAttributes={setAttributes} />

			<div id={attributes.id} className={classnames(className, 'wp-block-button')} style={buttonStyleParent}>
				{'none' !== attributes.iconType ? (
					<div className="wp-block-button__link" style={styles} css={hoverStyles}>
						{('left' === attributes.iconType || 'only' === attributes.iconType) &&
							('themeisle-icons' === attributes.library && attributes.icon ? (
								<Icon
									className={classnames({ 'margin-right': 'left' === attributes.iconType })}
									style={iconStyles}
								/>
							) : (
								<i
									className={classnames(attributes.prefix, 'fa-fw', `fa-${attributes.icon}`, {
						...
						{'right' === attributes.iconType &&
							('themeisle-icons' === attributes.library && attributes.icon ? (
								<Icon className="margin-left" style={iconStyles} />
							) : (
