...
const Save = ({ attributes, className }) => {
	...
	return (
		<div id={attributes.id} className={classnames(className, 'wp-block-button')}>
			<a
			...
			>
				{('left' === attributes.iconType || 'only' === attributes.iconType) &&
					('themeisle-icons' === attributes.library && attributes.icon ? (
						<Icon className={classnames({ 'margin-right': 'left' === attributes.iconType })} />
					) : (
						<i
				...
				{'only' !== attributes.iconType && <RichText.Content tagName="span" value={attributes.text} />}
