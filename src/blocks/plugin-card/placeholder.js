...
const { DOWN, ENTER, TAB, UP } = wp.keycodes;

const BlockPlaceholder = ({ setAttributes, hasError, setError, className }) => {
	...
	const searchPlugins = async (query) => {
		...
		let data = await apiFetch({
			path: `themeisle-gutenberg-blocks/v1/get_plugins?search=${encodeURIComponent(query)}`,
		});
		if (data.data.errors) {
			setError(true);
			setLoading(false);
	...
	const moveDown = (event) => {
		if (Object.keys(results).length) {
			...
			event.preventDefault();
			const nextIndex =
				null === selectedSuggestion || selectedSuggestion === Object.keys(results).length - 1
					? 0
