...
const { Button, Dashicon, Icon, Path, Placeholder, Rect, SVG, Tooltip } = wp.components;
...
const Onboarding = ({ clientId, setupColumns }) => {
	...
	return (
		...
		>
			...
			<Tooltip text={__('Open Template Library')}>
				...
				{isLibraryOpen && <Library clientId={clientId} close={() => setIsLibraryOpen(false)} />}
			</Tooltip>

			<div className="wp-block-themeisle-layout-skipper">
				<Button isLink onClick={() => setupColumns(1, 'equal')}>
					{__('Skip')}
				</Button>
			</div>
		</Placeholder>
