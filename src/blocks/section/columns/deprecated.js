...
const Separators = ({ type, front, style, fill, invert, width, height }) => {
	...
	return (
		<div
			className={classnames('wp-block-themeisle-blocks-advanced-columns-separators', type)}
			style={
				!front && width
					? {
...
const SeparatorsNew = ({ type, front, style, fill, invert, width, height }) => {
	if ('none' === style) {
		return false;
	}
		...
		>
			{'bigTriangle' === style && false === invert && (
				<SVG
					...
					preserveAspectRatio="none"
					xmlns="http://www.w3.org/2000/svg"
					className={classnames({ rotate: 'bottom' === type })}
				>
					<Path d="M0 0 L50 100 L100 0 Z"></Path>
				</SVG>
			)}
			...
			{'bigTriangle' === style && true === invert && (
				<SVG
					...
					preserveAspectRatio="none"
					xmlns="http://www.w3.org/2000/svg"
					className={classnames({ rotate: 'top' === type })}
				>
					<Path d="M100, 0l-50, 100l-50, -100l0, 100l100, 0l0, -100Z"></Path>
				</SVG>
			)}
...
const deprecated = [
	{
		...
		save: ({ attributes, className }) => {
			...
			if ('gradient' === attributes.backgroundType) {
				...
				background = {
					background: `${attributes.backgroundGradientType}-gradient( ${direction}, ${
						attributes.backgroundGradientFirstColor || 'rgba( 0, 0, 0, 0 )'
					} ${attributes.backgroundGradientFirstLocation}%, ${
						attributes.backgroundGradientSecondColor || 'rgba( 0, 0, 0, 0 )'
					} ${attributes.backgroundGradientSecondLocation}% )`,
				};
			}
			...
			if ('gradient' === attributes.backgroundOverlayType) {
				...
				overlayBackground = {
					background: `${attributes.backgroundOverlayGradientType}-gradient( ${direction}, ${
						attributes.backgroundOverlayGradientFirstColor || 'rgba( 0, 0, 0, 0 )'
					} ${attributes.backgroundOverlayGradientFirstLocation}%, ${
						attributes.backgroundOverlayGradientSecondColor || 'rgba( 0, 0, 0, 0 )'
					} ${attributes.backgroundOverlayGradientSecondLocation}% )`,
					opacity: attributes.backgroundOverlayOpacity / 100,
				};
			}
			...
			return (
				<Tag className={classes} id={attributes.id} style={style}>
					<div className="wp-themeisle-block-overlay" style={overlayStyle}></div>
	...
	},
	{
		...
		save: ({ attributes, className }) => {
			...
			return (
				<Tag className={classes} id={attributes.id}>
	...
	},
	{
		...
		migrate: (oldAttributes) => {
			...
			if ('gradient' === oldAttributes.backgroundType) {
				...
				backgroundGradient = `${oldAttributes.backgroundGradientType}-gradient(${direction}${
					hexToRgba(oldAttributes.backgroundGradientFirstColor) || 'rgba( 0, 0, 0, 0 )'
				} ${oldAttributes.backgroundGradientFirstLocation}%, ${
					hexToRgba(oldAttributes.backgroundGradientSecondColor) || 'rgba( 0, 0, 0, 0 )'
				} ${oldAttributes.backgroundGradientSecondLocation}%)`;
			}
			...
			if ('gradient' === oldAttributes.backgroundOverlayType) {
				...
				backgroundOverlayGradient = `${oldAttributes.backgroundOverlayGradientType}-gradient(${direction}${
					hexToRgba(oldAttributes.backgroundOverlayGradientFirstColor) || 'rgba( 0, 0, 0, 0 )'
				} ${oldAttributes.backgroundOverlayGradientFirstLocation}%, ${
					hexToRgba(oldAttributes.backgroundOverlayGradientSecondColor) || 'rgba( 0, 0, 0, 0 )'
				} ${oldAttributes.backgroundOverlayGradientSecondLocation}%)`;
			}
		...
		isEligible: (attributes) =>
			('gradient' === attributes.backgroundType && undefined !== attributes.backgroundGradientFirstColor) ||
			('gradient' === attributes.backgroundOverlayType &&
				undefined !== attributes.backgroundOverlayGradientFirstColor),
