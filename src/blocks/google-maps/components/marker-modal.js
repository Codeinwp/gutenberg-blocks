...
const { BaseControl, Button, ButtonGroup, Modal, SelectControl, TextControl } = wp.components;
...
const MarkerModal = ({ marker, isAdvanced, isPlaceAPIAvailable, addMarker, close }) => {
	useEffect(() => {
		setID(marker.id);
	...
	return (
		<Modal title={__('Add Marker')} onRequestClose={close} shouldCloseOnClickOutside={false}>
			{isAdvanced && (
				<Fragment>
					<BaseControl label={__('Location')} id={`themeisle-location-search-${marker.id}`}>
						<input
							type="text"
					...
					<TextControl label={__('Latitude')} type="text" value={latitude} onChange={setLatitude} />

					<TextControl label={__('Longitude')} type="text" value={longitude} onChange={setLongitude} />
				</Fragment>
			)}

			<TextControl label={__('Title')} type="text" value={title} onChange={setTitle} />

			<MarkerEditor label={__('Description')} type="text" value={description} onChange={setDescription} />
			...
			<ButtonGroup>
				...
				<Button isLarge isSecondary onClick={close}>
					{__('Cancel')}
				</Button>
			</ButtonGroup>
		</Modal>
