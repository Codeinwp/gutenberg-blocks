...
const Edit = ({ attributes, setAttributes, className, clientId, mergeBlocks, name, insertBlocksAfter, onReplace }) => {
	const { isViewportAvailable, isPreviewDesktop, isPreviewTablet, isPreviewMobile } = useSelect((select) => {
	...
	if (attributes.textShadow) {
		textShadowStyle = {
			textShadow: `${attributes.textShadowHorizontal}px ${attributes.textShadowVertical}px ${
				attributes.textShadowBlur
			}px ${hexToRgba(
				attributes.textShadowColor ? attributes.textShadowColor : '#000000',
				attributes.textShadowColorOpacity
			)}`,
		};
	}
					...
					fonts={[
						{
							...
							weights: attributes.fontVariant && [
								`${attributes.fontVariant + ('italic' === attributes.fontStyle ? ':i' : '')}`,
							],
						},
					]}
				/>
