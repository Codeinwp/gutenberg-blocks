...
const Edit = ({ attributes, setAttributes, className, isSelected, clientId }) => {
	...
	if (isEmpty(attributes.file)) {
		return <Placeholder className={className} value={attributes.file} onChange={onChangeFile} />;
	}

	return (
		<Fragment>
			<Inspector attributes={attributes} setAttributes={setAttributes} playerRef={playerRef} />

			<LottiePlayer attributes={attributes} className={className} isSelected={isSelected} playerRef={playerRef} />
		</Fragment>
	);
};
