...
const { PanelBody, Snackbar, ToggleControl } = wp.components;
...
const Options = () => {
			...
			if (false === isAPILoaded) {
				settingsRef.current.fetch().then((response) => {
					...
					if ('' !== response.themeisle_blocks_settings_global_defaults) {
						...
						window.themeisleGutenberg.globalDefaults = JSON.parse(
							response.themeisle_blocks_settings_global_defaults
						);
	...
	return (
		<Fragment>
			{canUser && (
				<PluginSidebarMoreMenuItem target="wp-block-themeisle-blocks-options">
					{__('Otter Options')}
				</PluginSidebarMoreMenuItem>
			)}

			<PluginSidebar title={__('Otter Options')} name="wp-block-themeisle-blocks-options">
