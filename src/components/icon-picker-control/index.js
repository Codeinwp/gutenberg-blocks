...
const IconPickerControl = ({ label, library, prefix, icon, changeLibrary, onChange }) => {
	...
	const FontAwesomeIconsList = ({ i, onToggle }) => {
		return (
			<MenuItem
				label={i.label}
				className={classnames({ 'is-selected': i.name === icon && i.prefix === prefix })}
				onClick={() => {
					onToggle();
					onChange({
						name: i.name,
			...
			>
				<i className={classnames(i.prefix, `fa-${i.name}`, 'fa-fw')}></i>
				{i.name}
			</MenuItem>
		);
	};

	const ThemeIsleIconsList = ({ i, onToggle }) => {
		return (
			<MenuItem
				label={i}
				className={classnames({ 'is-selected': i === icon })}
				onClick={() => {
					onToggle();
					onChange(i);
				}}
	...
	return (
		<BaseControl id={id} className="wp-block-themeisle-blocks-icon-picker-control">
			<Dropdown
								...
								{icon ? (
									<Fragment>
										{'fontawesome' === library &&
											(prefix ? (
												<Fragment>
													<i className={classnames(prefix, `fa-${icon}`, 'fa-fw')}></i>
													{icon}
												</Fragment>
											) : (
												__('Select Icon')
					...
					<MenuGroup label={'fontawesome' === library ? __('Font Awesome Icons') : __('ThemeIsle Icons')}>
						...
						<div className="components-popover__items">
							{selectedIcons.map((i) => {
								if (
									'fontawesome' === library &&
									(!search || i.search.some((o) => o.toLowerCase().match(search.toLowerCase())))
								) {
									return <FontAwesomeIconsList i={i} onToggle={onToggle} />;
								}

								if (
									'themeisle-icons' === library &&
									(!search || i.toLowerCase().match(search.toLowerCase()))
								) {
									return <ThemeIsleIconsList i={i} onToggle={onToggle} />;
								}
							})}
						</div>
					</MenuGroup>
