...
const HTMLAnchorControl = ({ value, onChange }) => {
	...
	const isInvalid =
		undefined !== window.themeisleGutenberg.blockIDs &&
		value !== ID &&
		window.themeisleGutenberg.blockIDs.includes(ID);
	...
	return (
		<InspectorAdvancedControls>
			...
			>
				<div className="wp-block-themeisle-blocks-html-anchor-control">
					...
					<Button
						...
						className={classnames('wp-block-themeisle-blocks-html-anchor-control-button', {
							'is-saved': !isEditing,
						})}
						onClick={() => {
			...
			{isInvalid && (
				...
				>
					{__(
						'This ID has already been used in this page. Please consider using a different ID to avoid conflict.'
					)}
				</Notice>
			)}
		</InspectorAdvancedControls>
